...
> 4. En **Astro-nettside** (`nyheter.modr.no`) viser dagens briefing + **arkiv**.
>
...
```
   ...
   nyheter.modr.no  TLS+hostname over    nyheter-web:8080
...
Mål: `https://nyheter.modr.no` viser dagens briefing som tekst, og et arkiv.
...
# 8. DNS: A-record nyheter.modr.no -> VPS-IP
...
- [ ] Etter DNS+proxy: `https://nyheter.modr.no` viser dagens tekst.
...
**Rapporter til plattform:** subdomene `nyheter.modr.no`, alias `nyheter-web`,
intern port `8080`.
```
